...
RSpec.describe 'お気に入り解除', type: :system do
  before do
    ...
    @manual2 = FactoryBot.create(:manual)
    @release2 = FactoryBot.create(:release, user_id: @manual2.user.id, manual_id: @manual2.id)
    @like1 = FactoryBot.create(:like, user_id: @manual2.user.id, manual_id: @manual1.id)
    @like2 = FactoryBot.create(:like, user_id: @manual1.user.id, manual_id: @manual2.id)
  end

  context 'お気に入り解除できるとき' do
    it 'お気に入り登録したユーザーはお気に入り解除できる' do
      # manual1のユーザーでログインする
      sign_in(@manual1.user)
      # トップページに公開されているマニュアルの情報が表示されている
      check_manual(@manual1.title, @manual1.category.name, @manual1.description)
      ...
      # お気に入りページに遷移する
      move_likepage(@manual1)
      # お気に入りページに先ほど解除したマニュアルの情報が存在しない
      check_no_manual(@manual2.title, @manual2.category.name, @manual2.description)
    end
  end

  context 'お気に入り解除できないとき' do
    it 'お気に入り登録したユーザー以外はお気に入り解除できない' do
